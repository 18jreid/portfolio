...
export default function Experience() {
  return (
    <section id="experience" className="py-16 md:py-24 px-6 md:px-16 lg:px-24 max-w-6xl mx-auto">
      ...
      >
        <p className="text-accent text-sm font-medium tracking-widest uppercase mb-2">01</p>
        <h2 className="text-3xl md:text-4xl font-bold text-prose mb-12">Experience</h2>

        <div className="relative">
          <div className="absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-accent/60 via-line to-transparent hidden md:block" />
          ...
          <div className="space-y-8">
            {jobs.map((job, i) => (
              ...
              >
                <div className={`absolute left-0 top-5 w-2 h-2 rounded-full -translate-x-[3px] hidden md:block ${
                  job.current ? "bg-accent shadow-[0_0_8px_#00e5ff]" : "bg-line border border-faint"
                }`} />

                <div className={`p-6 border transition-all duration-300 hover:border-accent/60 hover:bg-accent/5 ${
                  job.current ? "border-line border-l-2 border-l-accent bg-surface" : "border-line bg-surface"
                }`}>
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <h3 className="font-semibold text-prose text-base md:text-lg">{job.title}</h3>
                        {job.current && (
                          <span className="text-xs px-2 py-0.5 bg-green/10 border border-green/30 text-green rounded-sm">
                            Current
                          </span>
                        )}
                      </div>
                    ...
                    <div className="text-xs text-faint sm:text-right space-y-1 shrink-0">
                      <div className="flex items-center gap-1 sm:justify-end"><Calendar size={11} />{job.period}</div>
                      <div className="flex items-center gap-1 sm:justify-end"><MapPin size={11} />{job.location}</div>
                    </div>
                  </div>

                  <ul className="space-y-1.5 mb-4">
                    {job.bullets.map((b, j) => (
                      <li key={j} className="text-xs text-faint flex gap-2">
